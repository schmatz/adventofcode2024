...
for i in range((len(input_lines) + 1) // 4):
    ...
    spec = ProblemSpec(get_coord_from_line(aLine), get_coord_from_line(bLine), get_coord_from_line(prizeLine))
...
def optimized_int_combo(spec: ProblemSpec) -> tuple[int,int] | None:    
    buttons = np.array([[spec.ButtonA[0], spec.ButtonB[0]], [spec.ButtonA[1], spec.ButtonB[1]]])
...
altered_specs = [ProblemSpec(spec.ButtonA, spec.ButtonB, (spec.Prize[0] + OFFSET, spec.Prize[1] + OFFSET)) for spec in grid_specs]
