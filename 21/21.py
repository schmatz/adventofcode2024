...
def do_problem(filename: str, inner_robot_count: int) -> int:
    ...
    return optimized_manual_press_seq_length(numeric_keypad_codes, inner_robot_count)
...
def get_all_shortest_paths(
    ...
    candidate_paths = set(
        "".join(p) for p in permutations(element_string, r=manhattan_dist)
    )
...
@cache
def _calculate_keypad_paths(
    code: str, initial_button: str, keypad_type: Literal["NUMERIC", "DIRECTIONAL"]
) -> set[tuple[str, str]]:
    paths = (
        PREPROCESSED_NUMERIC_PATHS
        if keypad_type == "NUMERIC"
        else PREPROCESSED_DIRECTIONAL_PATHS
    )
...
def calculate_shortest_paths_numeric_keypad(
    code: str, initial_button: str
) -> set[tuple[str, str]]:
    return _calculate_keypad_paths(code, initial_button, "NUMERIC")


def calculate_shortest_paths_directional_keypad(
    code: str, initial_button: str
) -> set[tuple[str, str]]:
...
# Needs debugging
seqs_robot_a_029a = calculate_shortest_paths_directional_keypad(
    good_numeric_answer, "A"
)
...
def calculate_shortest_manual_press_sequence_length(
    codes: list[str], inner_robot_count: int
) -> int:
    canonical_robot_hand_positions: dict[int, str] = {
        x: "A" for x in range(inner_robot_count)
    }

    shortest_sequences = []
    for code in codes:
        shortest_seq_found = None
        shortest_robot_hand_positions: dict[int, str] = {
            x: "A" for x in range(inner_robot_count)
        }
        ...
        for numeric_seq, numeric_end_position in inner_loop_paths:
            ...
            def explore_paths(
                robot_id: int,
                previous_seq: str,
                working_robot_hand_positions: dict[int, str],
            ):
                ...
                if robot_id == inner_robot_count:
                    ...
                    if shortest_seq_found is None or len(previous_seq) < len(
                        shortest_seq_found
                    ):
                        shortest_seq_found = previous_seq
                        shortest_robot_hand_positions = working_robot_hand_positions
                    return
                directional_paths = calculate_shortest_paths_directional_keypad(
                    previous_seq, working_robot_hand_positions[robot_id]
                )
        ...
        print(
            f"For code {code} there is the seq {shortest_seq_found} with length {len(shortest_seq_found)}"
        )

    return sum(
        [
            calculate_sequence_complexity(short_seq[0], len(short_seq[1]))
            for short_seq in shortest_sequences
        ]
    )


@cache
def calculate_atomic_path_length(atom: str, levels_to_go: int) -> int:
    if levels_to_go == -1:
        return len(atom)  # This should represnt typing it in

    total_path_length = 0
    # We make the assumption that we are always restarting from A, TODO: double check this
    atomic_movements = "A" + atom

    for start_button, end_button in zip(atomic_movements, atomic_movements[1:]):
        if start_button == end_button:
            total_path_length += 1
            continue  # no path required
        paths_between_movements = {
            x + "A"
            for x in (
                PREPROCESSED_DIRECTIONAL_PATHS[(start_button, end_button)]
                if start_button != end_button
                else set()
            )
        }
        minimum_path_length = 1_000_000_000_000_000_000_000

        for atomic_path in paths_between_movements:
            candidate_path_length = calculate_atomic_path_length(
                atomic_path, levels_to_go - 1
            )

            if candidate_path_length < minimum_path_length:
                minimum_path_length = candidate_path_length

        total_path_length += minimum_path_length

    return total_path_length


def optimized_manual_press_seq_length(codes: list[str], inner_robot_count: int) -> int:
    code_shortest_sequence_length_tuples: list[tuple[str, int]] = []
    for numeric_code in codes:
        shortest_seq_length = None
        top_level_paths = calculate_shortest_paths_numeric_keypad(numeric_code, "A")
        for path, end_pos in top_level_paths:
            top_level_atoms = [c + "A" for c in path.split("A")[:-1]]

            atom_path_sum = 0
            for atom in top_level_atoms:
                atom_path_length = calculate_atomic_path_length(
                    atom, inner_robot_count - 1
                )
                atom_path_sum += atom_path_length

            if shortest_seq_length is None or atom_path_sum < shortest_seq_length:
                shortest_seq_length = atom_path_sum

        assert shortest_seq_length is not None
        code_shortest_sequence_length_tuples.append((numeric_code, shortest_seq_length))

    return sum(
        [
            calculate_sequence_complexity(short_seq[0], short_seq[1])
            for short_seq in code_shortest_sequence_length_tuples
        ]
    )
...
print("Part 2 solution", crazy_solution)
